...
def _check(
    ...
    checks.append({"check_name": check_name, "passed": passed, "details": details})
    if not passed:
...
async def reconcile_shipment(
    ...
    if invoice_total is None or agreed_rate is None:
        checks.append(
            {
                "check_name": "amount_variance",
                "passed": True,
                "details": "skipped: invoice or rate confirmation amount is missing",
            }
        )
    else:
    ...
    if not invoice_carrier or not rate_con_carrier:
        checks.append(
            {
                "check_name": "carrier_match",
                "passed": True,
                "details": "skipped: invoice or rate confirmation carrier is missing",
            }
        )
    else:
        _check(
            ...
            exception_reasons,
            "carrier_match",
            invoice_carrier == rate_con_carrier,
    ...
    if bol_pickup_date is None or rate_con_ship_date is None:
        checks.append(
            {
                "check_name": "bol_pickup_date",
                "passed": True,
                "details": "skipped: BOL pickup date or rate confirmation shipment date is missing",
            }
        )
    else:
    ...
    if shipment.has_pod:
        ...
        _check(
            ...
            bool(delivery_date and condition),
        ...
        checks.append(
            {
                "check_name": "pod_delivery_confirmation",
                "passed": True,
                "details": "POD not received yet, still within 3-day grace period",
            }
        )
    ...
    checks.append(
        {
            "check_name": "missing_docs",
            "passed": True,
            "details": ", ".join(missing_docs) if missing_docs else "none",
        }
    )
